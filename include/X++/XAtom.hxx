...
public: // functions
	...
	explicit XAtom(const Atom &atom) : m_atom(atom) {}
...
public: // functions
	 ...
	 **/
	XAtom getAtom(const std::string &s) const;

	//! Wrapper for getAtom(const std::string &) that uses const char*
	XAtom getAtom(const char *s) const { return this->getAtom(std::string(s)); }
...
protected: // functions
	...
	XAtom cacheMiss(const std::string &s) const;
